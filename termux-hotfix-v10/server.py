...
async def health(request:Request):
    ...
    return JSONResponse({
        ...
        "offline_ready":ready,
        "components":{"translator":llm,"stt":stt,"tts":tts},
        "ports":{"gateway":PORT,"translator":LLM_PORT,"stt":STT_PORT,"supertonic":SUPERTONIC_PORT,"piper":PIPER_PORT,"kokoro":KOKORO_PORT},
    })
...
async def translate_text(text:str,source:str,target:str,mode:str)->tuple[str,float]:
    # Dedicated seq2seq translation engine: no chain-of-thought, no chat prompt,
    # no sampling. This is intentionally optimized for low-latency speech translation.
    assert client
    t0=time.perf_counter()
    r=await client.post(f"{LLM_URL}/v1/translate",json={
        "text":text,
        "source_language":base_lang(source),
        "target_language":base_lang(target),
        "mode":mode,
    })
    if r.status_code>=400:
        raise RuntimeError(f"translator {r.status_code}: {r.text[:300]}")
    data=r.json()
