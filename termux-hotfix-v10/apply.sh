#!/data/data/com.termux/files/usr/bin/bash
set -euo pipefail
PAYLOAD="${1:?payload root required}"
BASE="https://raw.githubusercontent.com/emilalvaroserrano-collab/mastertrans/a027323da56d69a06058e62ce214232114b5550c"
echo "[compat-v10] Applying low-latency translation-only runtime — no Qwen / no chat LLM"

get(){ curl -fLsS --retry 6 --retry-delay 2 --retry-all-errors "$1" -o "$2"; }

# Core runtime
get "$BASE/termux-hotfix-v4/requirements.txt" "$PAYLOAD/gateway/requirements.txt"
get "$BASE/termux-hotfix-v10/tts_server.py" "$PAYLOAD/gateway/tts_server.py"
get "$BASE/termux-hotfix-v9/piper_server.py" "$PAYLOAD/gateway/piper_server.py"
get "$BASE/termux-hotfix-v9/kokoro_server.mjs" "$PAYLOAD/gateway/kokoro_server.mjs"

# v10 translator + STT + lifecycle
get "$BASE/termux-hotfix-v10/install_payload.sh" "$PAYLOAD/install.sh"
get "$BASE/termux-hotfix-v10/start.sh" "$PAYLOAD/start.sh"
get "$BASE/termux-hotfix-v10/stop.sh" "$PAYLOAD/stop.sh"
get "$BASE/termux-hotfix-v10/server.py" "$PAYLOAD/gateway/server.py"
get "$BASE/termux-hotfix-v10/translator_server.mjs" "$PAYLOAD/gateway/translator_server.mjs"
get "$BASE/termux-hotfix-v10/build_engines.sh" "$PAYLOAD/scripts/build_engines.sh"
get "$BASE/termux-hotfix-v10/download_models.sh" "$PAYLOAD/scripts/download_models.sh"
get "$BASE/termux-hotfix-v10/smoke_test.sh" "$PAYLOAD/scripts/smoke_test.sh"

# Current UI / provider settings
mkdir -p "$PAYLOAD/public"
get "$BASE/termux-hotfix-v6/index.html" "$PAYLOAD/public/index.html"
get "$BASE/termux-hotfix-v9/translate.html" "$PAYLOAD/public/translate.html"
get "$BASE/termux-hotfix-v9/settings.html" "$PAYLOAD/public/settings.html"
get "$BASE/termux-hotfix-v6/manifest.webmanifest" "$PAYLOAD/public/manifest.webmanifest"
get "$BASE/termux-hotfix-v9/service-worker.js" "$PAYLOAD/public/service-worker.js"

chmod +x "$PAYLOAD/install.sh" "$PAYLOAD/start.sh" "$PAYLOAD/stop.sh" "$PAYLOAD/scripts/"*.sh

bash -n "$PAYLOAD/install.sh"
bash -n "$PAYLOAD/start.sh"
bash -n "$PAYLOAD/stop.sh"
bash -n "$PAYLOAD/scripts/build_engines.sh"
bash -n "$PAYLOAD/scripts/download_models.sh"
bash -n "$PAYLOAD/scripts/smoke_test.sh"
python -m py_compile "$PAYLOAD/gateway/server.py" "$PAYLOAD/gateway/tts_server.py" "$PAYLOAD/gateway/piper_server.py"

grep -q -- '--convert' "$PAYLOAD/start.sh" || { echo "whisper browser-audio conversion missing"; exit 1; }
grep -q 'm2m100_418M' "$PAYLOAD/gateway/translator_server.mjs" || { echo "translation-only model missing"; exit 1; }
! grep -q 'Qwen' "$PAYLOAD/start.sh" || { echo "Qwen reference remains in runtime"; exit 1; }
! grep -q 'llama-server' "$PAYLOAD/start.sh" || { echo "llama runtime remains in realtime path"; exit 1; }
grep -q '/v1/translate' "$PAYLOAD/gateway/server.py" || { echo "translation endpoint missing"; exit 1; }

echo "[compat-v10] STT + translation-only engine + multi-TTS runtime verified"
