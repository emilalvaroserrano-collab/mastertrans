...
tail_fail(){ local name="$1"; echo; echo "ERROR: $name failed to become ready."; echo "--- $name.log ---"; tail -n 30 "$ROOT/logs/$name.log" 2>/dev/null || true; }
...
if quiet_ok "http://127.0.0.1:$EBURON_STT_PORT/"; then echo '  ✓ stt already ready'; else
  start_proc stt "exec '$ROOT/bin/whisper-server' --host 127.0.0.1 --port '$EBURON_STT_PORT' -m '$ROOT/models/$WHISPER_MODEL_NAME' -l auto -t '$WHISPER_THREADS'"
